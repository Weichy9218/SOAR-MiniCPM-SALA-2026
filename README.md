...
当前代码库已经从 **reproducibility scaffold** 推进到 **FP16 baseline smoke 可运行**。官方必需的 baseline args 已保留，当前为了稳定通过 Smax smoke，默认额外加 `--disable-cuda-graph`。
...
- full public accuracy 尚未跑完；`SOAR-Toolkit/eval_model.py` 当前写死 `max_out_len=65536`，不能把短 smoke 当作 accuracy 指标。
...
因此，当前阶段仍只推进 FP16 baseline：未进入 speculative decoding、LK loss 或量化实验。
...
6. 运行 full public accuracy；完成后再记录 `ori_accuracy`、`overall_accuracy`。
7. 准备官方 speed split 后设置 `SPEED_DATA_S1`、`SPEED_DATA_S8`、`SPEED_DATA_SMAX` 跑完整 S1/S8/Smax。
8. 更新 `artifacts/results/baseline_summary.md`。
...
8. smoke 通过后，跑 full public accuracy；准备官方 speed split 后再跑 full S1/S8/Smax。
9. 更新：
   - artifacts/results/baseline_summary.md
   - artifacts/results/quant_matrix.csv
